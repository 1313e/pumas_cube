...
Installation
------------
Before installation, one has to make sure that both the `Rubik's cube model`_ and the ``dev`` branch of the `PUMAS muon transport engine`_ libraries are correctly built as shared libraries.
